...
def reinhard_global(hdr, key=0.36):
...
def lift_shadows(img, lift=0.08):
...
def tone_map_real_estate(hdr_linear, brightness_key=0.36):
    img = hdr_linear.astype(np.float32) / 255.0
    img = reinhard_global(img, key=brightness_key)
    img = lift_shadows(img, lift=0.08)
    img = compress_highlights(img, ceiling=0.92)
    img = local_contrast_enhance(img, sigma=40.0, strength=0.30)
    img_hsv = cv2.cvtColor((img * 255).astype(np.uint8), cv2.COLOR_BGR2HSV)
    img_hsv[:, :, 1] = np.clip(img_hsv[:, :, 1].astype(np.float32) * 1.15, 0, 255).astype(np.uint8)
